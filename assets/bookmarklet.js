...
// Readable, un-minified bookmarklet source — single source of truth (no build).
// Bookmarklets accept multi-line code; it's URL-encoded when wired to the link.
const BOOKMARKLET_SRC = `(function () {
  var gf = (typeof g_form !== "undefined" && g_form)
        || (window.gsft_main && window.gsft_main.g_form)
        || (window.top && window.top.gsft_main && window.top.gsft_main.g_form);
  if (!gf) {
    alert("g_form not available. Open the ticket in ServiceNow and try again.");
    return;
  }

  var doc = (window.gsft_main && window.gsft_main.document) || document;
  var old = doc.getElementById("_aeExportBtn");
  if (old) old.remove();
  ...
  function safeVal(f) { try { return gf.getValue(f) || ""; } catch (e) { return ""; } }
  function safeDsp(f) { try { return gf.getDisplayValue(f) || gf.getValue(f) || ""; } catch (e) { return ""; } }
  function safeTbl() { try { return gf.getTableName ? gf.getTableName() : ""; } catch (e) { return ""; } }

  // Collect SLAs from the task_sla related list, drop admin SLAs, sort by highest %.
  function getSLA() {
    var doc2 = (window.gsft_main && window.gsft_main.document) || document;
    var EX = /Time to Transfer|Incident Age|Retention/i;
    var tables = Array.from(doc2.querySelectorAll('table[id*="task_sla"],table[data-list_id*="task_sla"]'));
    if (!tables.length) {
      tables = Array.from(doc2.querySelectorAll("table")).filter(function (t) {
        return t.querySelector(".percent_complete_text");
      });
    }
    ...
    tables.forEach(function (tbl) {
      Array.from(tbl.querySelectorAll("tbody tr")).forEach(function (row) {
        var pe = row.querySelector(".percent_complete_text");
        if (!pe) return;
        var pct = parsePct(pe.textContent);
        if (pct === null) return;
        var cells = Array.from(row.querySelectorAll("td.vt"));
        var name = cells.length ? txt(cells[0].textContent) : "";
        if (!name) name = txt(row.textContent).slice(0, 60);
        if (EX.test(name)) return;
        all.push({ name: name, pct: Math.round(pct * 10) / 10, breach: pct >= 100 ? "Yes" : "No" });
      });
    });
  ...
  btn.id = "_aeExportBtn";
  btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="vertical-align:middle;margin-right:6px"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Export Ticket';
  btn.style.cssText = "position:fixed;top:64px;right:28px;z-index:2147483647;padding:10px 16px;background:#4f46e5;color:white;border:none;border-radius:10px;cursor:grab;font-weight:700;font-size:13px;box-shadow:0 4px 18px rgba(79,70,229,.5);font-family:system-ui,sans-serif;display:flex;align-items:center;user-select:none;transition:box-shadow .15s";
  btn.onmouseenter = function () { btn.style.boxShadow = "0 6px 24px rgba(79,70,229,.7)"; };
  btn.onmouseleave = function () { btn.style.boxShadow = "0 4px 18px rgba(79,70,229,.5)"; };

  // Draggable, remembers its position across pages/sessions
  (function () {
    var KEY = "_aeBtnPos";
    try {
      var s = JSON.parse(localStorage.getItem(KEY) || "null");
      if (s && typeof s.left === "number") {
        btn.style.left = Math.max(0, Math.min(window.innerWidth - 60, s.left)) + "px";
        btn.style.top = Math.max(0, Math.min(window.innerHeight - 30, s.top)) + "px";
    ...
    doc.addEventListener("mousemove", function (e) {
      ...
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) btn._moved = true;
      var r = btn.getBoundingClientRect();
      btn.style.left = Math.max(0, Math.min(window.innerWidth - r.width, ox + dx)) + "px";
      btn.style.top = Math.max(0, Math.min(window.innerHeight - r.height, oy + dy)) + "px";
    });
    doc.addEventListener("mouseup", function () {
      ...
      try {
        localStorage.setItem(KEY, JSON.stringify({ left: parseInt(btn.style.left, 10), top: parseInt(btn.style.top, 10) }));
      } catch (e) {}
    });
  })();

  // Click → scrape the ticket and copy JSON
  btn.onclick = function (e) {
    if (btn._moved) { btn._moved = false; return; }
    var tbl = safeTbl();
    var sla = getSLA();
    var data = {
      table: tbl,
      number: safeVal("number"),
      shortDescription: safeVal("short_description"),
      assignedTo: (tbl === "sc_req_item" ? (safeDsp("opened_by") || safeDsp("assigned_to")) : safeDsp("assigned_to")),
      openedBy: safeDsp("opened_by"),
      assignmentGroup: safeDsp("assignment_group"),
      state: safeDsp("state"),
      requestItem: tbl === "sc_task" ? safeDsp("request_item") : "",
      parent: safeDsp("parent"),
      slas: sla || []
    };
    var json = JSON.stringify(data, null, 2);
    console.table(data);
    navigator.clipboard.writeText(json).then(function () {
      var msg = sla && sla.length
        ? sla.map(function (x) { return x.name + " - " + x.pct + "% - " + (x.breach === "Yes" ? "BREACH" : "OK"); }).join("\\n")
        : "No SLA found — expand the SLAs related list.";
      alert("Copied to clipboard!\\n\\n" + msg + "\\n\\nPaste in Agent Eval > Form > Paste ServiceNow.");
    }).catch(function () {
...
document.addEventListener("appReady", function () {
  const href = toBookmarkletUrl(BOOKMARKLET_SRC);
  document.getElementById("bookmarkletLink").href = href;

  document.getElementById("copyCodeBtn").addEventListener("click", function () {
    const code = toBookmarkletUrl(BOOKMARKLET_SRC);
    navigator.clipboard.writeText(code).then(function () {
      ...
      prompt("Copy this as the bookmark URL:", toBookmarkletUrl(BOOKMARKLET_SRC));
    });
  });
});
