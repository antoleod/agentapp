...
function bindForm() {
  ...
  document.getElementById("tempPassBtn")?.addEventListener("click", onCreateWithTempPassword);
...
// Faster onboarding path that does NOT depend on email delivery (useful when a
// firewall blocks Firebase mail). Creates the Auth account on a SECONDARY app so
// the admin's own session is untouched, then reuses the existing mustSetPassword
// flow to force the user to choose their own password on first login.

function generateTempPassword(len = 14) {
  // Unambiguous set (no 0/O, 1/l/I) so the password is safe to read aloud.
...
async function createAuthUserWithPassword(email, password) {
  // Secondary app: creating the account here does NOT sign the admin out.
  const secondary = firebase.initializeApp(firebase.app().options, "userCreator_" + Date.now());
  try {
...
async function onCreateWithTempPassword() {
  const email       = document.getElementById("inviteEmail").value.trim();
  const role        = document.getElementById("inviteRole").value;
  const displayName = document.getElementById("inviteNote").value.trim();
  const btn         = document.getElementById("tempPassBtn");
  ...
  try {
    await createAuthUserWithPassword(email, tempPassword);
    await setUserRole(email, displayName || null, role); // sets mustSetPassword: true
    document.getElementById("inviteForm").reset();
    document.getElementById("inviteRole").value = "evaluator";
    await refreshUsers();
    showTempPasswordModal(email, tempPassword);
  } catch (err) {
    const map = {
      "auth/email-already-in-use": "This email already has an account. Use the email invite, or have them reset via the recovery link — a temp password can only be set for new accounts.",
      "auth/invalid-email":        "Please enter a valid email address.",
      "auth/weak-password":        "Generated password was rejected. Please try again.",
    };
    toast(map[err.code] || ("Failed to create user." + (err.message ? " " + err.message : "")), "error");
  } finally {
    btn.disabled = false;
    btn.innerHTML = origHtml;
  }
...
function showTempPasswordModal(email, password) {
  const body = `
    <p style="font-size:14px;color:var(--text);line-height:1.6;margin:0 0 12px">
      Account created for <strong>${escapeHtml(email)}</strong>. Send this temporary password
      through a secure channel — it is shown <strong>only once</strong>. They will be required to
      set their own password the first time they sign in.
    </p>
    <div style="display:flex;gap:8px;align-items:center">
      <input id="tempPassValue" type="text" readonly value="${escapeHtml(password)}"
             style="flex:1;font-family:monospace;font-size:15px;letter-spacing:.5px" />
      <button class="btn btn-secondary" id="tempPassCopyBtn" type="button">Copy</button>
    </div>`;

  const overlay = showModal({
    title: "Temporary password",
    body,
    actions: [{ label: "Done", class: "btn-primary", onClick: m => closeModal(m) }],
  });

  const input   = overlay.querySelector("#tempPassValue");
  const copyBtn = overlay.querySelector("#tempPassCopyBtn");
