...
// Handles criterion HTML generation, score selection, picker value setting,
// and the average score badge. Used exclusively by the evaluation form.

const SCORE_VALUES = [1, 2, 3, 4, 5];

function buildScoreCriterion({ id, title, subtitle, value = 3 }) {
  const selectedValue = String(value);
  const hintStyle = getSettings().formShowScoreHints === false ? ' style="display:none"' : "";
  const scoreButtons = SCORE_VALUES.map(score => {
    const scoreValue = String(score);
    const isSelected = scoreValue === selectedValue;
    return `
        <button
          type="button"
          class="sp-btn${isSelected ? " sp-active" : ""}"
          data-v="${scoreValue}"
          role="radio"
          aria-checked="${isSelected}"
          aria-label="${escapeHtml(title)} score ${scoreValue}"
          tabindex="-1"
        >${scoreValue}</button>`;
  }).join("");
  ...
  return `
    <div
      ...
      tabindex="0"
      role="radiogroup"
      aria-label="${escapeHtml(title)} score"
      aria-describedby="criterion-${id}-hint"
    >
      <div class="criterion-label">
        <span>${escapeHtml(title)}</span>
        <span class="criterion-hint" id="criterion-${id}-hint"${hintStyle}>${escapeHtml(subtitle)}</span>
      </div>
      <div class="score-picker" data-field="${id}" data-value="${selectedValue}">
${scoreButtons}
      </div>
...
function renderAllCriteria() {
  ...
  getActiveCriteria().forEach(c => {
    ...
    container.insertAdjacentHTML("beforeend", buildScoreCriterion({
      id: c.id,
      title: label,
      subtitle: hint,
      value: getSettings().defaultScore !== undefined ? getSettings().defaultScore : 3,
    }));
  });

  container.querySelectorAll(".criterion").forEach(criterion => {
    criterion.addEventListener("keydown", handleCriterionKeydown);
  });
...
function handleCriterionKeydown(e) {
  const picker = e.currentTarget.querySelector(".score-picker");
  if (!picker) return;

  if (/^[1-5]$/.test(e.key)) {
    e.preventDefault();
    selectScore(picker, e.key);
    return;
  }

  const current = Number(picker.dataset.value || 3);
  if (e.key === "ArrowRight") {
    e.preventDefault();
    selectScore(picker, String(Math.min(current + 1, 5)));
    return;
  }

  if (e.key === "ArrowLeft") {
    e.preventDefault();
    selectScore(picker, String(Math.max(current - 1, 1)));
    return;
  }

  if (e.key === "Enter" || e.key === " ") {
    e.preventDefault();
    selectScore(picker, String(current));
  }
}
...
function selectScore(picker, value) {
  ...
  picker.querySelectorAll(".sp-btn").forEach(btn => {
    const isSelected = btn.dataset.v === value;
    btn.classList.toggle("sp-active", isSelected);
...
// ── SLA Toggle Component ──────────────────────────────────────────────────────
// Handles the SLA breach toggle buttons and the floating SLA group breach panel.
// Used exclusively by the evaluation form.
...
async function showSlaGroupPanel(anchorEl) {
  document.getElementById("slaGroupPanel")?.remove();

  const allData = typeof getData === "function" ? await getData() : [];
  const breaches = allData
    .filter(r => r.slaBreach === "Yes" && typeof r.lsa === "string" &&
                 r.lsa.toUpperCase().includes("ITEC") && r.lsa.toUpperCase().includes("SUP"))
    .sort((a, b) => (b.slaPercentage ?? 0) - (a.slaPercentage ?? 0))
    .slice(0, 10);

  const panel = document.createElement("div");
  panel.id = "slaGroupPanel";
  panel.style.cssText = "position:absolute;z-index:9999;background:#fff;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 8px 30px rgba(0,0,0,.15);padding:14px 16px;min-width:320px;max-width:420px;font-family:system-ui,sans-serif;font-size:13px";

  if (!breaches.length) {
    panel.innerHTML = `<div style="color:#6b7280;padding:8px 0">No SLA breaches found for ITEC-SUP groups.</div>`;
  } else {
    const rows = breaches.map(r => {
      const pct = r.slaPercentage != null ? `${r.slaPercentage}%` : "—";
      const color = (r.slaPercentage ?? 0) >= 150 ? "#b91c1c" : "#dc2626";
      return `<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #f3f4f6">
        <div>
          <div style="font-weight:600;color:#111">${escapeHtml(r.lsa)}</div>
          <div style="color:#6b7280;font-size:11px">${escapeHtml(r.agentName || "—")} · ${escapeHtml(r.ticketNumber || "—")}</div>
        </div>
        <div style="font-weight:700;color:${color};margin-left:12px">${pct}</div>
      </div>`;
    }).join("");
    panel.innerHTML = `<div style="font-weight:700;color:#111;margin-bottom:8px;font-size:12px;text-transform:uppercase;letter-spacing:.05em">Top SLA Breaches — ITEC SUP Groups</div>${rows}`;
  }

  const closeBtn = document.createElement("button");
  closeBtn.textContent = "✕";
  closeBtn.style.cssText = "position:absolute;top:8px;right:10px;background:none;border:none;cursor:pointer;font-size:14px;color:#9ca3af";
  ...
  const dismiss = e => { if (!panel.contains(e.target) && e.target !== anchorEl) { panel.remove(); document.removeEventListener("click", dismiss); } };
  setTimeout(() => document.addEventListener("click", dismiss), 0);
}

let _editCreatedAt = null;
let _editPrevData  = null;
let _editUpdatedAt = null; // millis of the loaded record's updatedAt, for conflict detection
let _selectedSla   = null; // { name, pct, breach } of the SLA persisted with the evaluation
...
document.addEventListener("appReady", async () => {
  ...
  populateAgentRoster();
  bindEvents();
  loadSlaDefinitions().catch(() => {}); // team SLA blocklist for the picker
  restoreDraft(); // restore an unsaved draft if autosave is enabled
  ...
  if (ticket) {
    ...
    if (item) {
      ...
      _editPrevData  = { ...item };
      _editUpdatedAt = tsToMillis(item.updatedAt);
      loadIntoForm(item);
      updateProgress();
      updateAvgBadge();
    } else {
      ...
      try {
        ...
        if (doc.exists) {
          ...
          _editPrevData  = { ...data };
          _editUpdatedAt = tsToMillis(data.updatedAt);
          loadIntoForm(data);
          updateProgress();
          updateAvgBadge();
...
// ── Draft autosave (opt-in via Settings) ──────────────────────────────────────
const DRAFT_KEY = "agentEvalDraftV1";
let _draftTimer = null;

function saveDraft() {
  if (!getSettings().autoSaveDraft) return;
  if (_editCreatedAt !== null) return; // don't draft while editing an existing record
  clearTimeout(_draftTimer);
  _draftTimer = setTimeout(() => {
    const d = readForm();
    const meaningful = d.ticketNumber || d.agentName || d.comments ||
      getAllScoreFields().some(id => { const v = document.getElementById(id)?.value; return v && v !== "3"; });
    if (meaningful) localStorage.setItem(DRAFT_KEY, JSON.stringify(d));
  }, 600);
}
...
function restoreDraft() {
  ...
  try {
    ...
    loadIntoForm(d);
    _editCreatedAt = null; // a draft is a new form, not an edit
    updateProgress();
    toast(t("form.draft_restored"), "info");
...
function readForm() {
  ...
  const item = {
    ...
    slaName:            _selectedSla?.name || "",
    slaPercentage:      _selectedSla?.pct ?? null,
    lsa:                v("lsa").trim(),
  ...
  // Freeze the average over the criteria scored now, so it stays stable
  // regardless of the viewer's active criteria later.
  item.avgScore = calculateAverage(item);
  return item;
}
...
function loadIntoForm(item) {
  ...
  _selectedSla = (item.slaName || item.slaPercentage != null)
    ? { name: item.slaName || "", pct: item.slaPercentage, breach: item.slaBreach || "No" }
    : null;
...
async function pasteServiceNowInfo() {
  ...
  // Clear previous data before loading new ticket
  _editCreatedAt = null;
  _editPrevData  = null;
  clearForm();
  ...
  } else if (data.assignedTo || data.openedBy) {
    agentEl.value = data.assignedTo || data.openedBy;
    filled.push("Agent Name");
  }

  // SLA — multi picker or single auto-set
  const slas = normalizeSlas(data);
  applySlas(slas, filled, skipped);
  ...
  updateProgress();
  showAuditPanel(data, filled, slas);
...
// ── SLA helpers ───────────────────────────────────────────────────────────────
function normalizeSlas(data) {
  if (Array.isArray(data.slas) && data.slas.length) return data.slas;
  // backwards compat with old single-sla format
  if (data.slaBreach) return [{ name: data.slaName || "", pct: data.slaPercentage || 0, breach: data.slaBreach }];
  return [];
}

function applySlas(slas, filled, skipped) {
  if (!slas.length) { skipped.push("SLA Breach (set manually)"); return; }
  // Default to the highest business % that isn't blocked by the team.
  const pick = slas.find(s => !isSlaBlocked(s.name)) || slas[0];
  _selectedSla = pick; // selectSlaOption may override
  setSlaValue(pick.breach);
  const tag = slas.length > 1 ? " · " + t("sla.highest") : "";
  filled.push(`SLA Breach (${pick.pct}%) → ${pick.breach === "Yes" ? "⚠ Breached" : "✓ OK"}${tag}`);
}

function showAuditPanel(data, filled, slas = []) {
  // Remove existing panel
  document.getElementById("auditPanel")?.remove();

  // Auto-collect newly-seen SLAs into the team-wide list (fire-and-forget).
  if (slas.length) registerSlas(slas.map(s => s.name)).catch(() => {});
  ...
  if (!slas.length)                                      remaining.push("SLA Breach");
  const allScoresDefault = getAllScoreFields().every(id => {
    const v = document.getElementById(id)?.value;
    return !v || v === "3";
  });
  if (allScoresDefault) remaining.push(`Scores (${getAllScoreFields().length} criteria) — all at default 3`);

  // SLA picker as the 3rd column. Blocked SLAs are hidden (managed in Settings).
  const visibleSlas = slas.filter(s => !isSlaBlocked(s.name));
  const selIdx = Math.max(0, visibleSlas.findIndex(s => _selectedSla && s.name === _selectedSla.name && s.pct === _selectedSla.pct));
  const slaColHtml = visibleSlas.length > 1 ? `
      <div class="audit-col audit-col-sla">
        <div class="audit-col-title">${t("sla.pick")}</div>
        <div class="audit-sla-hint">${escapeHtml(t("sla.manage_hint"))}</div>
        <div class="audit-sla-list">
        ${visibleSlas.map((s, i) => {
          const color  = s.breach === "Yes" ? "var(--danger)" : "var(--success)";
          const icon   = s.breach === "Yes" ? "⚠" : "✓";
          const pctSafe = escapeHtml(String(s.pct));
          const nameSafe = escapeHtml(s.name);
          return `<button type="button" class="audit-sla-option${i === selIdx ? " audit-sla-option--selected" : ""}" data-idx="${i}"
            style="border-left:3px solid ${color}">
            <span class="audit-sla-pct" style="color:${color}">${icon} ${pctSafe}%</span>
            <span class="audit-sla-name">${nameSafe}</span>
          </button>`;
        }).join("")}
        </div>
      </div>` : "";
  ...
  const topSla = slas[0];
  const slaDisplay = topSla ? (() => {
    const color = topSla.breach === "Yes" ? "color:var(--danger);font-weight:700" : "color:var(--success);font-weight:700";
    return `<span style="${color}">${escapeHtml(String(topSla.pct))}% ${topSla.breach === "Yes" ? "⚠ Breached" : "✓ OK"}</span>`;
  })() : "";

  // RITM/REQ don't carry their own SLA — it lives on the related SCTASK.
  // When importing a request-level ticket with no SLA, tell the evaluator to use the SCTASK.
  const tnum = (data.number || data.ticketNumber || "").toUpperCase();
  const tbl  = data.table || "";
  const isReqLevel = tnum.startsWith("RITM") || tnum.startsWith("REQ") ||
                     tbl === "sc_req_item" || tbl === "sc_request";
  const slaNote = (isReqLevel && slas.length === 0) ? `
    <div class="audit-sla-warning" style="margin:10px 14px 4px;padding:10px 14px;border-radius:8px;background:rgba(245,158,11,.12);border:1px solid var(--warning);color:var(--text);font-size:13px;line-height:1.5">
      ${escapeHtml(t("sla.reqlevel_note"))}
    </div>` : "";
  ...
  panel.innerHTML = `
    <div class="audit-header">
      ...
      <button type="button" class="audit-collapse" aria-label="Collapse" title="Minimize / expand">
        <svg class="toggle-collapse-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M6 9l6 6 6-6"/></svg>
      </button>
      <button type="button" class="audit-close">×</button>
    </div>
    <div class="audit-body${slaColHtml ? " audit-body-3" : ""}">
      <div class="audit-col">
        <div class="audit-col-title audit-done">✓ Auto-filled</div>
        ${filled.map(f => `<div class="audit-item audit-item-done">${escapeHtml(f)}</div>`).join("")}
      </div>
      <div class="audit-col">
        <div class="audit-col-title audit-todo">→ Still needed</div>
        ${remaining.map(f => `<div class="audit-item audit-item-todo">${escapeHtml(f)}</div>`).join("")}
      </div>
      ${slaColHtml}
    </div>
    ${slaNote}
  ...
  // Store the visible (non-blocked) slas on panel for selectSlaOption
  panel._slas = visibleSlas;

  // Insert above the form card
  const form = document.getElementById("evaluationForm");
  form.parentNode.insertBefore(panel, form);

  // Wire handlers (inline onclick is blocked by the page CSP).
  panel.querySelector(".audit-close")?.addEventListener("click", () => panel.remove());
  panel.querySelector(".audit-collapse")?.addEventListener("click", () => {
    panel.classList.toggle("audit-panel--collapsed");
  });
  panel.querySelectorAll(".audit-sla-option").forEach(btn => {
    btn.addEventListener("click", () => selectSlaOption(Number(btn.dataset.idx)));
  });

  panel.scrollIntoView({ behavior: "smooth", block: "nearest" });
}

function selectSlaOption(idx) {
  const panel = document.getElementById("auditPanel");
  const slas  = panel?._slas;
  if (!slas || !slas[idx]) return;
  const s = slas[idx];
  if (isSlaBlocked(s.name)) { toast(`${t("sla.blocked_toast")}: ${s.name}`, "warning"); return; }
  _selectedSla = s;
  setSlaValue(s.breach);
  document.getElementById("lsa").value = s.name;
  updateProgress();
  // Highlight selected
  panel.querySelectorAll(".audit-sla-option").forEach((btn, i) => {
    btn.classList.toggle("audit-sla-option--selected", i === idx);
  });
  toast(`${t("sla.selected")}: ${s.name} (${s.pct}%)`, "success");
}
...
async function handleSave(e) {
  ...
  // Duplicate guard: warn before overwriting an existing evaluation when creating new.
  if (_editCreatedAt === null && await evaluationExists(item.ticketNumber)) {
    if (!confirm(`${item.ticketNumber}: ${t("form.dup_confirm")}`)) return;
  }

  // Concurrency guard: when editing, warn if the record changed elsewhere since load.
  if (_editCreatedAt !== null && _editUpdatedAt !== null) {
    const current = await getEvaluationUpdatedAt(item.ticketNumber);
    if (current !== null && current !== _editUpdatedAt && !confirm(t("form.conflict_confirm"))) {
      return;
    }
  }
